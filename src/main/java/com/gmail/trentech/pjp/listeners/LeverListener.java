	...
	public void onChangeBlockEventModify(ChangeBlockEvent.Modify event, @Root Player player) {
		...
		try {
			for (Transaction<BlockSnapshot> transaction : event.getTransactions()) {
				...
				if (lever.getServer().isPresent()) {
					Consumer<String> consumer = (server) -> {
						Server teleportEvent = new TeleportEvent.Server(player, server, lever.getServer().get(), lever.getPrice(), Cause.of(NamedCause.source(lever)));
					...
					Optional<Location<World>> optionalSpawnLocation = lever.getLocation();
